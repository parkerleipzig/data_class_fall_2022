...
median_pct_change <- mdcity_income %>% 
...
Beaver Creek had the highest absolute change. Shaft had the highest percent change.
```{r}

absolute_change <- mdcity_income %>% 
  mutate(abs_change = median_inc_2020 - median_inc_2010) %>% 
  slice_max(abs_change, n=10)
...
```{r}
highest_pct_change <- median_pct_change %>% 
...
```{r}
 lowest_pct_change <-median_pct_change %>% 
...
To find the median value of the percentage change, you would find the summary statistics of the column pct_change in the median_pct_change table and see the median from there. To make a table with the 10 neighborhoods with the highest and lowest change, we could create two tables using the slice_max and slice_min like we did for questions 2 and 3, (but change n to 10) and join the tables together using the merge function.
