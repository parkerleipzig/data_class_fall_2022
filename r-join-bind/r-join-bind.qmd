...
Answer: Because the data for the name column in each dataframe has different entries (Kent County, Maryland
versus just Kent County) this will result in one of the risks of joining dataframes which is that the rows will be double counted and there will be two entries of the same county with the data rather than just one 
