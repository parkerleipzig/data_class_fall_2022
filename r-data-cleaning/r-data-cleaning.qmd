...
#Answer: To ensure the consistency of the state name in this dataframe. I would first ensure there are no differences between upper and lowercase versions of the state by using the upper() function to convert all rows in the state column to uppercase. Then I would utilize the casewhen() function to indicate that anytime it says "MARYLAND" it should be converted to MD. This would make all of the state names have the same output in the dataframe. 
